...
By default (`query: false`) **each endpoint is a plain typed fetch** — call it and you
...
Set `query: true` and every endpoint instead returns a **handle** carrying its own
...
// query: true: ReturnType<typeof api.users.show> is the handle
```
