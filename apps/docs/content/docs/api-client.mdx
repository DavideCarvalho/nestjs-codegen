...
Register the [`nestjsInertiaCodegen()`](/docs/inertia) extension and the generated
client additionally exposes a typed `navigate()` helper backed by the Inertia `router`,
so links and mutations can drive Inertia visits. See [nestjs-inertia](/docs/inertia).
