...
| ------------- | --------------------- | ----------- |
| `'comma'` (default) | `?ids=a,b` | Back-compat: the historical behavior. The server must split the joined value itself. |
| `'repeat'` | `?ids=a&ids=b` | The form NestJS's default (Express `qs`) parser revives as a `string[]` — it matches the codegen's generated `Array<string>` query type without server-side normalization. |

The default is `'comma'` so upgrading is non-breaking. Set `'repeat'` at the fetcher level
for every request, or per request to override:
...
<Callout type="info">
  Whichever format you send, the **single-value** case is still a bare string on the wire
  (`?ids=a`), and Express hands the server back a `string`, not a `string[]`. See
  [Receiving array query params](/docs/client/array-query-params) for how to receive them
  safely (and why `ParseArrayPipe` is the wrong tool for optional array params).
</Callout>
