...
| --- | --- | --- |
| `tanstackQuery()` | `@dudousxd/nestjs-codegen-tanstack` | Wraps each leaf into a handle exposing `queryOptions`/`mutationOptions`/`infiniteQueryOptions`/`queryKey`. See [TanStack Query](/docs/tanstack-query). |
| `nestjsFilterCodegen()` | `@dudousxd/nestjs-filter-codegen` | Adds a typed `filterQuery()` member to leaves whose route is decorated with `@ApplyFilter`/`@FilterFor`. See [nestjs-filter](/docs/filters). |
| `nestjsInertiaCodegen()` | `@dudousxd/nestjs-inertia-codegen-extension` | Adds the Inertia `router` import and a typed `navigate()` helper to `api.ts`. See [nestjs-inertia](/docs/inertia). |
