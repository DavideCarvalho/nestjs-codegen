...
<Callout type="info">
  If dev and CI ever resolve genuinely *different* configs for the same `outDir`, `generate()`
  throws instead of silently overwriting the other's output — see
  [Drift guard](/docs/configuration#drift-guard).
</Callout>
