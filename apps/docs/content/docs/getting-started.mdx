...
[nestjs-filter](/docs/filters)) and `nestjsInertiaCodegen()` (Inertia `router` +
`navigate()`, see [nestjs-inertia](/docs/inertia)). Leave the array empty for a plain
typed-fetch client. See [Extensions](/docs/extensions).
...
<Callout type="info">
  ...
  Inertia app (see [nestjs-inertia](/docs/inertia)). A plain NestJS API needs just
  `contracts` + `codegen`.
</Callout>
