...
@Module({
  imports: [
    NestjsCodegenModule.forRoot({
      ...
      query: false,               // true → TanStack Query handles (see below)
    }),
  ],
})
...
Want **TanStack Query**? Set `query: true` and each call returns a handle with
...
<Callout type="info">
  Default `query: false` keeps the client TanStack-free (plain fetch). Not using
  Inertia? `mutationClient: 'fetcher'` drops the `@inertiajs/*` import too.
</Callout>
