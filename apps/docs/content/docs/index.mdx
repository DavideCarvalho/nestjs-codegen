...
- **Bring your own fetcher** — native `fetch` by default, or an axios instance; superjson
  and transformer pipelines supported. See [Fetcher](/docs/fetcher).
- **Extension-based integrations** — register integrations in `extensions: [...]`; each
  ...
  - **TanStack Query** (`tanstackQuery()`) — emits `queryOptions`/`mutationOptions` from
    *your* adapter package (react/vue/svelte/solid). See [TanStack Query](/docs/tanstack-query).
  - **nestjs-filter** (`nestjsFilterCodegen()`) — `@FilterFor`/`@ApplyFilter` → typed
    `filterQuery()` helpers. See [Filters](/docs/filters).
  - **nestjs-inertia** (`nestjsInertiaCodegen()`) — pages, shared props, and the Inertia
    `router` + `navigate()` helper. See [Inertia](/docs/inertia).
...
<Cards>
  ...
  <Card title="API client" href="/docs/api-client" description="The createApi(fetcher) factory and the typed call surface." />
  <Card title="Fetcher" href="/docs/fetcher" description="Bring your own transport — native fetch, axios, and superjson." />
  <Card title="TanStack Query" href="/docs/tanstack-query" description="Opt-in queryOptions / mutationOptions handles in api.ts." />
  <Card title="Extensions" href="/docs/extensions" description="How integrations register and shape the generated client." />
  <Card title="Programmatic" href="/docs/programmatic" description="loadConfig, generate, and watch — the codegen as a library." />
</Cards>
