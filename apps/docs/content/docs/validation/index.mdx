...
with a **validation adapter**. zod ships in the box; valibot and arktype are separate
packages. Adding a new library is one `render` function away (the interface is shaped
...
export default defineConfig({
  validation: 'zod', // bundled — nothing else to install
});
```
